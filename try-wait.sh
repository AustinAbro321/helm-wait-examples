...
# $helm_path uninstall basic-legacy --wait=legacy -n legacy
# kubectl wait --for=delete deployment/basic-legacy-podinfo -n legacy --timeout=1s

$helm_path uninstall basic-watcher -n watcher
kubectl get pods -n watcher podinfo
kubectl wait --for=delete pod/podinfo -n watcher --timeout=1s
kubectl wait --for=delete deployment/basic-watcher-podinfo -n watcher --timeout=1s
